...
struct File;
  ...
  // 
  File getFileFromFileName(string name){return File();}
  ...
  void findAllFiles(string rootPath,vector<string>&names, vector<File>&Files){
    ...
    names.insert(names.end(), this_names.begin(), this_names.end());
    files.insert(files.end(), this_files.begin(), this_Files.end());
